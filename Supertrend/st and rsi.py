...
from Backtest.st2 import apply_pivots, custom_resample
...
def preprocess_data(data):
    ...
    data_hourly["SUPERTREND"] = ta.supertrend

    data["Date"] = data.index.date
    data["datetime"] = data.index

    merged_data = pd.merge(
        data, data_hourly[["RSI"]], left_index=True, right_index=True, how="left"
    ).ffill()
    ...
    merged_data = pd.merge(
        merged_data,
        daily_data[
            ["Date", "PP", "TC", "BC", "R1", "R2", "R3", "S1", "S2", "S3", "%CPR"]
        ],
        left_on="Date",
        right_on="Date",
        how="left",
...
def define_trading_conditions(data):
    ...
    data["Signal"] = np.select(
        [buy_condition, sell_condition, short_condition, cover_condition],
...
def backtest(
    df, initial_capital=100000, risk_per_trade=0.01, stop_loss=0.01, take_profit=0.03
):
    ...
    for index, row in df.iterrows():
        ...
        # Handle Short signals for short positions
            ...
            trades["Quantity"].append(qty)
            logger.info(
                f"Short Signal: {index}, Price: {row['Close']}, Quantity: {qty}"
            )
        ...
        if position == "Long":
            if (
                row["Signal"] == "Sell"
                or row["Shifted_close"] < (1 - stop_loss) * trades["Buy Price"][-1]
                or row["Shifted_close"] > (1 + take_profit) * trades["Buy Price"][-1]
                or index.time() > close_time
            ):
                trades["Sell Date"][-1] = index
                trades["Sell Price"][-1] = row["Close"]
                capital += qty * row["Close"]
                logger.info(
                    f"Sell Signal: {index}, Price: {row['Close']}, PnL: {qty * (row['Close'] - trades['Buy Price'][-1])}"
                )
                position = None
        ...
        # Handle Cover signals or stop-loss/take-profit for short positions
            ...
            if (
                row["Signal"] == "Cover"
                or row["Shifted_close"] > (1 + stop_loss) * trades["Sell Price"][-1]
                or row["Shifted_close"] < (1 - take_profit) * trades["Sell Price"][-1]
                or index.time() > close_time
            ):
                ...
                logger.info(
                    f"Cover Signal: {index}, Price: {row['Close']}, PnL: {qty * (trades['Sell Price'][-1] - row['Close'])}"
                )
                position = None
...
def calculate_trade_metrics(trade_history_df):
    ...
    trade_history_df["Realized Profit"] = (
        trade_history_df["PnL"] * trade_history_df["Quantity"]
    )
    trade_history_df["Cum Profit"] = trade_history_df["Realized Profit"].cumsum()
    trade_history_df["Cumulative Return"] = (
        1 + trade_history_df["Return"]
    ).cumprod() - 1
    trade_history_df["Drawdown"] = (
        trade_history_df["Cum Profit"].cummax() - trade_history_df["Cum Profit"]
    )
    trade_history_df["Days"] = (
        trade_history_df["Sell Date"] - trade_history_df["Buy Date"]
    ).dt.days
...
def process_file(file_path):
    ...
    trade_history_df.to_csv(
        os.path.join(output_dir, f"{file_name}_trade_history.csv"), index=False
    )
...
def main(folder_path):
    ...
    with ThreadPoolExecutor() as executor:
        future_to_file = {
            executor.submit(process_file, file): file for file in stock_files
        }
